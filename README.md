# Novelintel v1.0
<p>Developed by Novelintel Investigations</p><br>

# What is Novelintel?  
<p>Novelintel is a comprehensive toolkit designed for expanding, gathering and harvesting Information aswell as network analysis. Including tools that require close physical proximity, such as captive portals, as well as tools that allow remote attacks, such as brute‑force methods or network scans. One notable component is its web module, inspired by Hound (developed by TechChipNet). While Hound focuses primarily on geolocation capture, Novelintel expands the concept by offering a much more comprehensive website and many more ways of additional data‑collection vectors such as device sensors and credential‑capture workflows. The interface has been extensively redesigned, retaining only the core payload structure and conversational layout.</p><br>

<p>Beyond its web‑based features, Novelintel provides streamlined installation and usage of essential security tools including Nmap, Hydra, John the Ripper, and many others. The toolkit operates through a command‑line interface and is optimized for Debian‑based distributions to ensure reliability and minimize configuration issues. The tool relies primarily on Open-source intelligence (OSINT) and social engineering to gather information. Utilities like Nmap or Hydra can extend its capabilities, but I rarely use them myself. Included tools are installed via Snap or APT to ensure proper PATH integration and consistent behavior. Novelintel also offers optional installation of reputable open‑source security projects such as Airgeddon and Fluxion, which significantly enhance wireless assessment capabilities. The toolkit brings together a diverse collection of utilities ranging from widely used industry standards to lesser‑known niche tools acknowledging that different practitioners rely on different methodologies and workflows.</p><br>

## Obtainable information from the web page
<ul>
  <li>Camera Access</li>
  <li>GPS Coordinates</li>
  <li>Logging files</li>
  <li>Microphone Input</li>
  <li>Message logs</li>
  <li>User Credentials</li>
  <li>System Information</li>
  <li>Different ways of IP capture</li>
</ul><br>

## Cookie Copier Browser extension
<p>Is a lightweight privacy and data‑control tool that lets you view, export, filter, clear and import your browser cookies with ease.  
The extension collects your browsing data on demand, Exports is in different formats and lets you import cookies. Encoding the cookies makes it hard to read meaning if you have no idea what a cookie is don't accidentally paste it into a website that logs messages ;). When importing, it automatically detects all domains and displays them in a searchable, alphabetized list for quick navigation. You can open any domain in a new tab, inspect its presence in your cookie data, or clear all cookies from your browser with a single action. A built‑in theme toggle, smooth animations, and a compact interface make the tool fast and pleasant to use.</p><br>

## Obtainable information via the multi-tool
<p>The tool itself has a method for gathering any type of information, the important part is that you know what you are doing and are targeting someone vulnerable to specific attacks.</p><br>

## Features  
<p>The tool offers a wide range of features and functionality, including:</p><br>
<ul>
  <li>HOST AND CREATE</li>
  <li>OPEN DATA FOLDER</li>
  <li>LAUNCH MAP VIEWER</li>
  <li>EXPLOITATION</li>
  <li>OPEN SOURCE INTELLIGENCE</li>
  <li>INSTALL ALL REQUIREMENTS</li>
  <li>IP LOOKUP</li>
  <li>ADD TARGET</li>
  <li>PASSWORD ATTACKS</li>
  <li>EXPLOITATION</li>
  <li>NETWORK UTILITIES</li>
  <li>PROTOCOLS AND REMOTE ACCESS</li>
  <li>MAN IN THE MIDDLE</li>
  <li>DIGITAL FORENSICS</li>
  <li>PACKET SNIFFERS</li>
  <li>INSTALL GITHUB TOOLS</li>
</ul><br>

## Supported Platforms  
<ul>
  <li>Kali Linux</li>
  <li>Ubuntu</li>
  <li>Linux Mint</li>
  <li>Pop!_OS</li>
  <li>Parrot Sec OS</li>
  <li>Other Debian-based distributions</li>
</ul><br>

## Requirements  
<p>Installation is simplified through the CLI and Novelintel.sh, select the "INSTALL ALL REQUIREMENTS" option and thats it. Tools installed from GitHub may need a different installation process than those in the Manuals folder, because some Debian distributions require using a virtual environment.</p><br>

## Usage Tips  
<p>Web pages are most effective when accessed on mobile devices. Certain features, such as geolocation, may be less accurate on PCs or laptops, which also often lack webcams, TikTok and Instagram do not have native desktop applications. Preferred browsers are Firefox, Edge, Chrome, and Safari, as other browsers may block certain features.</p><br>

## Important Notice  
<p>Novelintel is intended only for ethical and educational purposes. Unauthorized or malicious use of this tool is strictly discouraged. Please respect privacy and legal boundaries when conducting security research.</p><br>

## Developer notice
<p>AI tools were used to support development, but the project’s architecture, functionality and logic were designed and validated by me. AI served as a coding assistant, not the author of the tool.</p>
